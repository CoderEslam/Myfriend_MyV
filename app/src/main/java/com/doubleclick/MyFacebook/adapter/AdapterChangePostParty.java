package com.doubleclick.MyFacebook.adapter;
...
import com.doubleclick.MyFacebook.GetTimeAgo;
import com.doubleclick.MyFacebook.R;
import com.doubleclick.MyFacebook.model.ModelPost;
import com.doubleclick.MyFacebook.profile.UserProfileActivity;
import com.doubleclick.MyFacebook.watchParty.ChangeWatchPartyActivity;
import com.doubleclick.MyFacebook.watchParty.StartPartyActivity;
import com.doubleclick.MyFacebook.watchParty.StartYouTubeActivity;
...
public class AdapterChangePostParty extends RecyclerView.Adapter<AdapterChangePostParty.MyHolder>{
    ...
    public void onBindViewHolder(@NonNull MyHolder holder, int position) {
        ...
        //UserInfo
        FirebaseDatabase.getInstance().getReference().child("Users").child(modelPosts.get(holder.getAdapterPosition()).getId()).addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                holder.dp.setOnClickListener(v -> {
                    if (!modelPosts.get(holder.getAdapterPosition()).getId().equals(Objects.requireNonNull(FirebaseAuth.getInstance().getCurrentUser()).getUid())){
                        Intent intent = new Intent(context, UserProfileActivity.class);
                        intent.putExtra("hisUID", modelPosts.get(holder.getAdapterPosition()).getId());
        ...
        //Time
        long lastTime = Long.parseLong(modelPosts.get(holder.getAdapterPosition()).getpTime());
        holder.time.setText(GetTimeAgo.getTimeAgo(lastTime));

        //Extra
        FirebaseDatabase.getInstance().getReference("postExtra").child(modelPosts.get(holder.getAdapterPosition()).getpId()).addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()){
                    ...
                    if (Objects.requireNonNull(snapshot.child("privacy").getValue()).toString().equals("No one")){
                        ...
                        FirebaseDatabase.getInstance().getReference().child("Follow").child(modelPosts.get(holder.getAdapterPosition()).getId())
                                .child("Followers").addListenerForSingleValueEvent(new ValueEventListener() {
                            @Override
                            public void onDataChange(@NonNull DataSnapshot snapshot) {
                                if (!snapshot.hasChild(Objects.requireNonNull(FirebaseAuth.getInstance().getCurrentUser()).getUid()) &&  !modelPosts.get(holder.getAdapterPosition()).getId().equals(FirebaseAuth.getInstance().getCurrentUser().getUid())){
                                    ViewGroup.LayoutParams params = holder.itemView.getLayoutParams();
                                    params.height = 0;
                                    holder.itemView.setLayoutParams(params);
                                }
        ...
        //PostDetails
        String type = modelPosts.get(holder.getAdapterPosition()).getType();
        if (!type.equals("bg")){
            holder.text.setLinkText(modelPosts.get(holder.getAdapterPosition()).getText());
        }

        if (type.equals("video")){
            holder.mediaView.setVisibility(View.VISIBLE);
            holder.play.setVisibility(View.VISIBLE);
            Glide.with(context).asBitmap().load(modelPosts.get(holder.getAdapterPosition()).getVine()).thumbnail(0.1f).into(holder.mediaView);
        }
        ...
        holder.itemView.setOnClickListener(v -> {
            if (type.equals("video")){
                ...
                hashMap.put("link", modelPosts.get(holder.getAdapterPosition()).getVine());
